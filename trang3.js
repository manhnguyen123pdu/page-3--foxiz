...
for (m = 0; m < moon.length; m++) {
    moon[m].onclick = function () {
        ...
        nav2.classList.add('background--black')
...
for (s = 0; s < sun.length; s++) {
    sun[s].onclick = function () {
        for(b=0;b<background.length;b++){
            background[b].classList.remove('background--black')
        }
        for (n = 0; n < moon.length; n++) {
        ...
        nav2.classList.remove('background--black')
    }
}
